...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns:tei="http://www.tei-c.org/ns/1.0"
      ...
      <div class="large-10 columns details">
        ...
        <xsl:if test="//tei:support/tei:objectType[@xml:lang=$lang][@n = $n or not(@n or $n)]">
          <div class="row">
            ...
            <div class="large-9 columns">
              <p>
                <xsl:apply-templates select="//tei:objectType[@xml:lang=$lang][@n = $n or not(@n or $n)]"/>
                <xsl:text>&#160;</xsl:text>
              </p>
            </div>
          </div>
        ...
        <xsl:if test="//tei:support/tei:material[@xml:lang=$lang][@n = $n or not(@n or $n)]">
          <div class="row">
            ...
            <div class="large-9 columns">
              <p>
                <xsl:apply-templates select="//tei:material[@xml:lang=$lang][@n = $n or not(@n or $n)]"/>
                <xsl:text>&#160;</xsl:text>
              </p>
            </div>
          </div>
  ...
  <xsl:template name="editions_author_name">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$author/tei:forename">
        <xsl:value-of
          select="normalize-space($author//tei:forename[if (not(@xml:lang)) then true() else @xml:lang=$lang][1])"
        />
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of
          select="normalize-space($author//tei:*[if (not(@xml:lang)) then true() else @xml:lang=$lang][1])"
        />
      </xsl:otherwise>
    </xsl:choose>
  ...
  <xsl:template match="tei:bibl">
    ...
    <xsl:for-each select="//bib//tei:biblStruct[@xml:id=$target]|//bib//tei:bibl[@xml:id=$target]">
      ...
      <xsl:if
        test="descendant::tei:analytic//tei:author[3] or 
